...
module orbit_integrator
  ...
  integer, parameter :: numPerturbers = 3

  ! set these logical variables for either backwards or forwards integration
  logical, parameter :: backward = .TRUE.  ! integrating backwards                                              
  logical, parameter :: forward = .FALSE.  

  !Everything in cgs units.  In order:  LMC, SMC, Sgr
!  double precision, dimension(numPerturbers), parameter :: massp = (/2D10*Msun, 2D9*Msun, 3D10*Msun /)  ! fiducial masses
  double precision, dimension(numPerturbers), parameter :: massp = (/2D10*Msun, 2D9*Msun, 3D9*Msun /)  ! 10X lower Sgr mass
  double precision, dimension(numPerturbers), parameter :: scalelengthp = (/0.6D0*kpc, 0.6D0*kpc, 0.6D0*kpc/)
  double precision, dimension(numPerturbers), parameter :: cp = (/9.4D0, 9.4D0, 9.4D0/)
  double precision, dimension(numPerturbers) :: vp200

! double precision, dimension(numPerturbers), parameter :: xp0 = (/51.438D0*kpc, -8.76D0*kpc, -51.689*kpc/), &
!       yp0 = (/269.10D0*kpc, 190.39D0*kpc, -8.614D0*kpc/), zp0 = (/-73.283D0*kpc, 2.465*kpc, 23.25*kpc/)
!  double precision, dimension(numPerturbers), parameter :: vxp0 = (/-35.56D0*kms, 37.379D0*kms, -157.40D0*kms/), &
!       vyp0 = (/-247.58D0*kms, -142.91D0*kms, -8.847D0*kms/), vzp0 = (/5.089D0*kms, -76.802D0*kms, -30.143D0*kms/)

! from HST proper motions. In order:  LMC, SMC, Sgr
  double precision, dimension(numPerturbers), parameter :: xp0 = (/-1D0*kpc, 13.99D0*kpc, 14.67*kpc/), &
       yp0 = (/-40.45D0*kpc, -34.89D0*kpc, 2.27D0*kpc/), zp0 = (/-26.62D0*kpc, -40.5D0*kpc, -5.85*kpc/)
  double precision, dimension(numPerturbers), parameter :: vxp0 = (/-81D0*kms, -65.81D0*kms, 228D0*kms/), &
       vyp0 = (/-314D0*kms, -242D0*kms, -21D0*kms/), vzp0 = (/204D0*kms, 117D0*kms, 201D0*kms/)

  double precision, dimension(numPerturbers,3) :: posp ! x,y,z of perturbers
  double precision, dimension(numPerturbers,3) :: vposp ! vx,vy,vz of perturbers
...
program nbody
  ...
  external :: integrate_orbit, nfwmass, hernquistmass, writedata, & 
  ...
  double precision, dimension(numPerturbers,6) :: rstartp
...
!  startingTime = 1D0*Gyr ! integrate backward by this time
  startingTime = 1.D0*Gyr ! integrate forwards
   ...
   if (forward) then
      call get_starting_conditions(startingTime, rstartp)   ! integrate forwards
   endif
  ...
  do i = 1, numPerturbers
     write(*,fmt="(I5,';',6(E12.5';'))") i, posp(i,:)/kpc, vposp(i,:)/kms
  end do
  pause ' ' 
...
!     Jm(1,1,k) = 0.1D0*v0*semi
  ...
  do tau = 0D0, tau_end, tdyn
     ...
     r0 = 10D0*kpc
     call writedata(tau/Gyr, numparts, rparts)
     
!     write(*,*) frac*tau/tdyn, rparts(1,1)/lengthscale, rparts(1,5)/lengthscale
        
     call fourierdata(tau, numparts, rparts)
...
subroutine initializedata
  ...
  open(unit=22,file="orbit.out",status="old")
  open(unit=25,file="fourier.out",status="old")
...
subroutine get_starting_conditions(startingTime, rstartp)
  ...
  double precision, intent(OUT), dimension(numPerturbers,6) :: rstartp
  ...
  integer, parameter :: fine = 20
  
  rpertu(:,1) = sqrt(xp0*xp0 + yp0*yp0) ! setup r
  theta = atan2(yp0,xp0) ! setup theta
  rpertu(:,2) = cos(theta)*vxp0 + sin(theta)*vyp0 ! setup vr
  rpertu(:,3) = xp0*vyp0 - yp0*vxp0 ! setup angular momentum
  rpertu(:,4) = theta(:)
  rpertu(:,5) = zp0
  rpertu(:,6) = vzp0
  ...
  do while(t < startingTime)
     ...
     rpertu = rpertuout
     t = t + tau
     if (t+tau > startingTime) tau = startingTime-t
  end do
  endif
...
subroutine fourierdata(tau, numparts, rparts)
  use rings
  use orbit_integrator

  implicit none
  integer :: numparts, i, k, tindex, rindex, m
  double precision :: tau, dphi, a0, a0tot, norma0
  integer, parameter :: NUMMODES = 5
  double precision, dimension(NUMMODES) :: amcos, amsin, amcostot, amsintot
  double precision, dimension(numparts,6) :: rparts
...
subroutine writedata(tau, numparts, rparts)
  implicit none
  integer :: numparts, i
  ...
  do i = 1, numparts, 1
     ...
     y = r*sin(theta)
     z = rparts(i,5)
     write(22,fmt='(6(1X,E12.4))') 1D0*i, x, y, z, r, theta
...
!     sigma=58D0*kms  ! median 1-d velocity dispersion for Vmax=215 km/s halo in fdf.pro, following Zentner & Bullock 03. 
     ...
     accdf(1) = -4*pi*Gn**2*(log(Lambda))*rhoh*(1./vscalar**3)*vpos(1)*&
      (erf-2*xchandra*(1./sqrt(pi))*(exp(-(xchandra**2))))*msat

     accdf(2) = -4*pi*Gn**2*(log(Lambda))*rhoh*(1./vscalar**3)*vpos(2)* &
      (erf-2*xchandra*(1./sqrt(pi))*(exp(-(xchandra**2))))*msat
 
     accdf(3) = -4*pi*Gn**2*(log(Lambda))*rhoh*(1./vscalar**3)*vpos(3)* &
      (erf-2*xchandra*(1./sqrt(pi))*(exp(-(xchandra**2))))*msat

  end if  ! ends dynamical friction consideration - add on below if needed.

     if (gc.le.numPerturbers) then
         acc(1) = -(Gn*mass)*pos(1)/(absr*absr*absr) + accdf(1)
         acc(2) = -(Gn*mass)*pos(2)/(absr*absr*absr) + accdf(2)
         acc(3) = -(Gn*mass)*pos(3)/(absr*absr*absr) + accdf(3)
     else 
